...
-- Job completion does not prove that equipment is physically back. Keep
-- that uncertainty explicit until an intake scan records a warehouse zone.
UPDATE devices d
...
-- Legacy on_job values without an issued job are not trustworthy. Preserve
-- a known warehouse location; otherwise expose the uncertainty.
UPDATE devices d
SET status = CASE
...
BEGIN
    ...
    IF warehouse_job_status_is_closed(new_status_name) THEN
        ...
        WHERE jd.jobid = NEW.jobid
          AND jd.deviceid = d.deviceid
          AND jd.pack_status = 'issued'
          AND d.status IN ('on_job', 'rented')
          AND NOT EXISTS (
              SELECT 1
              FROM job_devices other_jd
              ...
              WHERE other_jd.deviceid = d.deviceid
                AND other_jd.jobid <> NEW.jobid
