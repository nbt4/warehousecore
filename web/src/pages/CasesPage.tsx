...
export function CasesPage() {
  ...
  const [jobs, setJobs] = useState<Job[]>([]);
	const [caseModels, setCaseModels] = useState<Array<{ model_id: number; name: string; case_count: number }>>([]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
	  const [caseResult, locationResult, jobResult, modelResult] = await Promise.all([
        handlingUnitsApi.list({ search: searchQuery || undefined, workflow_status: statusFilter || undefined }),
		warehouseApi.locations(false), jobsApi.getAll(), handlingUnitsApi.models(),
      ]);
	  ...
	  setCaseModels(modelResult.data);
  ...
  const openEdit = (item: HandlingUnit) => {
	...
	setForm({ name: item.name, description: item.description, case_type: item.case_type, case_model_id: item.case_model_id, width: item.width, height: item.height, depth: item.depth, weight: item.weight, max_weight_kg: item.max_weight_kg, zone_id: item.zone_id, home_zone_id: item.home_zone_id, barcode: item.barcode, rfid_tag: item.rfid_tag });
    setFormOpen(true);
  };
  ...
  return <div className="space-y-5">
    ...
    <form onSubmit={(event) => { event.preventDefault(); setSearchQuery(search.trim()); }} className="flex flex-col gap-3 rounded-xl border p-3 sm:flex-row" style={{ borderColor: 'var(--border-subtle)', background: 'var(--bg-card)' }}><div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2" style={{ color: 'var(--text-muted)' }} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name, Barcode oder Case-ID" className="w-full rounded-lg border py-2.5 pl-10 pr-3" style={fieldStyle} /></div><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="rounded-lg border px-3 py-2.5" style={selectStyle}><option value="" style={optionStyle}>Alle Status</option>{Object.entries(workflowLabels).map(([value,label]) => <option key={value} value={value} style={optionStyle}>{label}</option>)}</select><button className="rounded-lg px-4 py-2 font-semibold" style={{ background: 'var(--accent-red)', color: 'var(--text-primary)' }}>Suchen</button></form>
	{formOpen && <CaseEditor form={form} setForm={setForm} locations={locations} caseModels={caseModels} editing={Boolean(editingId)} onSubmit={saveCase} onClose={() => setFormOpen(false)} />}
