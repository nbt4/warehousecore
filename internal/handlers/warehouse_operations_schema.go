...
const warehouseOperationsSchemaVersion = "039_warehouse_operations_and_handling_units"

// EnsureWarehouseOperationsSchema upgrades legacy storage zones and cases to
// professional locations and handling units. It is intentionally idempotent so
// an installation can safely start without a separate migration runner.
func EnsureWarehouseOperationsSchema() error {
	...
	tx, err := db.Begin()
	if err != nil {
		return fmt.Errorf("begin warehouse operations schema transaction: %w", err)
	}
	defer tx.Rollback()
	...
	statements := []string{
		...
		`DO $$ BEGIN
		 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_storage_zones_capacity_positive') THEN
		   ALTER TABLE storage_zones ADD CONSTRAINT chk_storage_zones_capacity_positive
		...
		`DO $$ BEGIN
		 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_cases_workflow_status') THEN
		   ALTER TABLE cases ADD CONSTRAINT chk_cases_workflow_status CHECK (workflow_status IN ('empty','packing','complete','sealed','staged','on_job','return_check','maintenance'));
		 END IF; END $$`,
		...
		`DO $$ BEGIN
		 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_devices_storage_zone') THEN
		   ALTER TABLE devices ADD CONSTRAINT fk_devices_storage_zone FOREIGN KEY (zone_id) REFERENCES storage_zones(zone_id) ON DELETE SET NULL NOT VALID;
		 END IF; END $$`,
		`INSERT INTO warehouse_schema_migrations (version) VALUES ('039_warehouse_operations_and_handling_units') ON CONFLICT (version) DO NOTHING`,
	}

	for _, statement := range statements {
		if _, err := tx.Exec(statement); err != nil {
			return fmt.Errorf("apply warehouse operations schema: %w", err)
		}
	}

	if err := tx.Commit(); err != nil {
		return fmt.Errorf("commit warehouse operations schema: %w", err)
	}
	return nil
}
