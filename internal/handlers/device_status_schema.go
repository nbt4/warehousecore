...
const deviceStatusSchemaVersion = "040_device_status_and_condition"

// EnsureDeviceStatusSchema installs the canonical device lifecycle. status is
// exclusively the physical material-flow state; condition_status independently
// controls whether the device is operationally usable.
func EnsureDeviceStatusSchema() error {
	...
	statements := []string{
		...
		`CREATE OR REPLACE FUNCTION warehouse_job_status_is_closed(job_status TEXT)
		 RETURNS BOOLEAN AS $$ SELECT LOWER(TRIM(COALESCE(job_status, ''))) IN (
		...
		`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='chk_devices_physical_status') THEN
		 ALTER TABLE devices ADD CONSTRAINT chk_devices_physical_status CHECK (status IN ('in_storage','on_job','return_pending','location_unknown'));
		 END IF; END $$`,
		`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='chk_devices_condition_status') THEN
		 ALTER TABLE devices ADD CONSTRAINT chk_devices_condition_status CHECK (condition_status IN ('available','blocked','defective','maintenance','retired'));
		 END IF; END $$`,
		...
		`CREATE TABLE IF NOT EXISTS device_status_history (
			...
			previous_zone_id INT,new_zone_id INT,previous_location VARCHAR(255),new_location VARCHAR(255),
			change_source VARCHAR(50) NOT NULL DEFAULT 'database',changed_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
		)`,
		`CREATE INDEX IF NOT EXISTS idx_device_status_history_device_time ON device_status_history(device_id,changed_at DESC)`,
		...
		`CREATE TRIGGER devices_audit_status_change BEFORE UPDATE OF status,condition_status,zone_id,current_location ON devices
		...
		`CREATE OR REPLACE FUNCTION sync_devices_after_job_status_change() RETURNS TRIGGER AS $$
		 ...
		 BEGIN
			...
			ELSE
				...
				FROM job_devices jd WHERE jd.jobid=NEW.jobid AND jd.deviceid=d.deviceid AND jd.pack_status='issued' AND d.status='return_pending';
			END IF; RETURN NEW;
		 END; $$ LANGUAGE plpgsql`,
		`DROP TRIGGER IF EXISTS jobs_sync_device_status ON jobs`,
		`CREATE TRIGGER jobs_sync_device_status AFTER UPDATE OF statusid ON jobs FOR EACH ROW
		 WHEN (OLD.statusid IS DISTINCT FROM NEW.statusid) EXECUTE FUNCTION sync_devices_after_job_status_change()`,
		`INSERT INTO warehouse_schema_migrations(version) VALUES ('040_device_status_and_condition') ON CONFLICT(version) DO NOTHING`,
	}
