...
const maintenanceSchemaVersion = "042_maintenance_work_management"

// EnsureMaintenanceSchema installs the canonical maintenance work-management
// model. It is intentionally idempotent because WarehouseCore may start on a
// database that only contains one of the legacy defect schemas.
func EnsureMaintenanceSchema() error {
	...
	tx, err := db.Begin()
	if err != nil {
		return fmt.Errorf("begin maintenance schema transaction: %w", err)
	}
	defer tx.Rollback()
	...
	statements := []string{
		`CREATE TABLE IF NOT EXISTS maintenance_plans (
			...
			CONSTRAINT chk_maintenance_plan_type CHECK (maintenance_type IN ('preventive','inspection','calibration')),
			CONSTRAINT chk_maintenance_plan_interval CHECK (interval_days BETWEEN 1 AND 3650),
			CONSTRAINT chk_maintenance_plan_lead_time CHECK (lead_time_days BETWEEN 0 AND 365),
		...
		`CREATE TABLE IF NOT EXISTS maintenance_orders (
			...
			created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
			updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
			CONSTRAINT chk_maintenance_order_type CHECK (order_type IN ('defect','preventive','inspection','calibration')),
			CONSTRAINT chk_maintenance_order_priority CHECK (priority IN ('low','normal','high','critical')),
			CONSTRAINT chk_maintenance_order_status CHECK (status IN ('open','planned','in_progress','waiting_parts','completed','cancelled')),
			CONSTRAINT chk_maintenance_order_outcome CHECK (outcome IS NULL OR outcome IN ('passed','passed_with_notes','failed','repaired')),
			CONSTRAINT chk_maintenance_order_cost CHECK (cost IS NULL OR cost >= 0)
		)`,
	...
	for _, statement := range statements {
		if _, err := tx.Exec(statement); err != nil {
			return fmt.Errorf("apply maintenance schema: %w", err)
		}
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("commit maintenance schema: %w", err)
	}
	return nil
}
