...
func EnsureLabelStudioSchema() error {
	...
	tx, err := db.Begin()
	if err != nil {
		return fmt.Errorf("begin label studio migration: %w", err)
	}
	defer tx.Rollback()

	statements := []string{
		`ALTER TABLE label_templates ADD COLUMN IF NOT EXISTS target_type VARCHAR(20) NOT NULL DEFAULT 'device'`,
		`ALTER TABLE label_templates ADD COLUMN IF NOT EXISTS revision INTEGER NOT NULL DEFAULT 1`,
		`CREATE INDEX IF NOT EXISTS idx_label_templates_target ON label_templates(target_type)`,
		`CREATE UNIQUE INDEX IF NOT EXISTS uq_label_templates_default_target ON label_templates(target_type) WHERE is_default`,
		...
		`INSERT INTO label_assets (target_type, target_id, label_path, source_updated_at)
		 SELECT 'device', deviceid, label_path, updated_at FROM devices
		...
		`INSERT INTO warehouse_schema_migrations (version) VALUES ('034_label_studio_and_direct_print') ON CONFLICT (version) DO NOTHING`,
	}
